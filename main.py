...
def batchToString(batch: Batch):
  ...
  for i, order_id in enumerate(batch.orders):
    ...
    reverse_marker = "'" if batch.is_order_reversed[i] else ""
        
    # Calculate the route makespan
    route_makespan = calculate_route_makespan(global_order_list[order_id].route, distance_matrix)
        
    # Append to the string
    s += f" o{order_id}({global_order_list[order_id].route}){reverse_marker}[{route_makespan}] ,"
    
  # Add the final part of the string
  s += " 0"
  batch_makespan = calculate_batch_makespan(batch.orders, batch.is_order_reversed)
  s += f", Makespan: [{batch_makespan}]"
...
def simulated_annealing_batch_orders(batch: Batch, initial_temperature_batch_orders, cooling_rate_batch_orders, L_batch_orders, K2_batch_orders, epsilon2_batch_orders):
    ...
    while not terminate:
        ...
        for i, order_id in enumerate(best_batch_route):
          # Determine if the order is reversed
          reverse_marker = "'" if best_batch_reverse[i] else ""
          
          # Append to the string
          s += f" o{order_id}({global_order_list[order_id].route}){reverse_marker} ,"
...
def assign_orders_to_batches(order_list, batch_list: List[Batch]):
    for order_id in order_list:
      ...
      batches_sorted = sorted([(n[0], n[1].order_route_makespan_sum()) for n in enumerate(batch_list)], key=lambda n: n[1])
      batch_indexes_sorted_by_order_route_makespan = [bs[0] for bs in batches_sorted]

      for bi in batch_indexes_sorted_by_order_route_makespan:
        b = batch_list[bi]
      ...
      if not assigned:
        # print(f"Batch list size before extenstion, {len(batch_list)}")
        new_batches = [Batch() for _ in range(Batch.num_pickers)]
        if not new_batches[0].assign(global_order_list[order_id].items):
           print("Order does not fit to an empty batch")
           exit()
        batch_list.extend(new_batches)
...
def main(args):
  ...
  Batch.num_pickers = args.num_pickers
  ...
  for order_id in range(num_orders):
    ...
    o.route_makespan = calculate_route_makespan(o.route, distance_matrix)
