...
class Batch():
  ...
  def __init__(self, V=(1200, 1000, 1500)):
  ...
  def __repr__(self):
     return f"Batch[{self.id}]: Orders:{self.orders} W: {self.weight()}"

  def reset_num_batches():
     Batch.num_batches = 0

  def weight(self):
     return sum([global_order_list[order_id].weight for order_id in self.orders])

  def order_route_makespan_sum(self):
     return sum([global_order_list[order_id].route_makespan for order_id in self.orders])
  ...
  def assign(self, items_in_order):
    ...
    if self.weight() + order_weight > Batch.max_weight:
       return False
